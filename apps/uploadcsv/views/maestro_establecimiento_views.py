...
from apps.uploadcsv.models import MAESTRO_HIS_ESTABLECIMIENTO
from apps.uploadcsv.serializers import MAESTRO_HIS_ESTABLECIMIENTOSerializer
...
class MAESTRO_HIS_ESTABLECIMIENTO_CSV_View(APIView, FileValidationMixin):
    ...
    def post(self, request):
        ...
        instance = MAESTRO_HIS_ESTABLECIMIENTO()
        identifier_field = "Id_Establecimiento"
        try:
           # leer
...
class MAESTRO_HIS_ESTABLECIMIENTO_Delete_View(APIView):
    permission_classes = [AllowAny]

    def delete(self, request):
        with connection.cursor() as cursor:
            # Trunca la tabla para eliminar todos los registros y reiniciar los contadores de secuencia
            table_name = MAESTRO_HIS_ESTABLECIMIENTO._meta.db_table
            cursor.execute(
...
class MAESTRO_HIS_ESTABLECIMIENTO_List_View(generics.ListAPIView):

    queryset = MAESTRO_HIS_ESTABLECIMIENTO.objects.all()
    serializer_class = MAESTRO_HIS_ESTABLECIMIENTOSerializer
